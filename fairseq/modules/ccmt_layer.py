...
class TransformerDeliberaterLayer(TransformerDecoderLayer):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.decoder_attn = self.build_encoder_attention(self.embed_dim, args)
    ...
    def forward(
        ...
        residual = x
        if self.normalize_before:
            x = self.decoder_attn_layer_norm(x)
        if prev_attn_state is not None:
            prev_key, prev_value = prev_attn_state[:2]
            saved_state: Dict[str, Optional[Tensor]] = {
                "prev_key": prev_key,
